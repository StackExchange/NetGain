...
namespace StackExchange.NetGain
{
    public class TcpServer : TcpHandler
    {
        ...
        {
            log = LogManager.Current.GetLogger<TcpServer>();
        }
        ...
        public void Stop()
        {
            ...
            if (proc != null)
            {
                log.Info("{0}\tShutting down connections...", Connection.GetLogIdent());
                foreach (var conn in allConnections)
                {
                    ...
                    { log.Error("{0}\t{1}", Connection.GetIdent(conn), ex.Message); }
                }
                Thread.Sleep(100);
            }
            foreach (var conn in allConnections)
            {
                ...
                {
                    try { socket.Close(); }
                    catch (Exception ex) { log.Error("{0}\t{1}", Connection.GetIdent(conn), ex.Message); }
            ...
            if (connectSockets != null)
            {
                foreach (var tuple in connectSockets)
                {
                    ...
                    try
                    {
                        ...
                        log.Info("{0}\tService stopping: {1}", Connection.GetConnectIdent(endpoint), endpoint);
                        connectSocket.Close();
                    }
                    catch (Exception ex) { log.Error("{0}\t{1}", Connection.GetConnectIdent(endpoint), ex.Message); }
        ...
        public void Start(string configuration, params IPEndPoint[] endpoints)
        {
            ...
            {
                log.Info("{0}\tService starting: {1}", Connection.GetConnectIdent(endpoints[i]), endpoints[i]);
        ...
        void ResurrectListenerAlreadyHaveLock(int i)
        {
            ...
            try
            {
                log.Error("Restarting listener on " + endpoint + "...");
                var newSocket = StartAcceptListener(endpoint);
                connectSockets[i] = Tuple.Create(newSocket, endpoint);
                if (newSocket == null)
                {
                    log.Error("Unable to restart listener on " + endpoint + "...");
                }
                else
                {
                    log.Error("Restarted listener on " + endpoint + "...");
                }
            }
            catch (Exception ex)
            {
                log.Error("Restart failed on " + endpoint + ":" + ex.Message);
            }
        }
        protected override void OnAcceptFailed(SocketAsyncEventArgs args, Socket socket)
        {
            ...
            try
            {
                ...
                log.Error("Listener failure: " + args.SocketError);
            ...
            {
                log.Error("Epic fail in OnAcceptFailed: " + ex.Message);
            }
        }
        ...
        private Socket StartAcceptListener(EndPoint endpoint)
        {
            ...
            {
                log.Error("Unable to start listener on " + endpoint.ToString() + ": " + ex.Message);
                return null;
            }
        }
        ...
        public void Heartbeat()
        {
            lock(heartbeatLock) // don't want timer and manual invoke conflicting
            {
                ...
                if (tmp != null)
                {
                    ...
                    {
                        log.Error("{0}\tHeartbeat: {1}", Connection.GetHeartbeatIdent(), ex.Message);
                    }
                }
